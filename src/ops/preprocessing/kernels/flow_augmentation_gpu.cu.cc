...
bool FillFlowAugmentation(const GPUDevice& device,
                          ...
                          typename TTypes<float,
                                          2>::ConstTensor transforms_from_b,
                          typename TTypes<float,
                                          2>::ConstTensor inv_transforms_from_a) {
  ...
  FillFlowAugmentationKernel << < config.block_count, config.thread_per_block, 0,
    device.stream() >> > (
    total_count / 2, flows.data(), transforms_from_b.data(),
    inv_transforms_from_a.data(),
