...
class DataAugmentation : public OpKernel {
  public:
    ...
    void Compute(OpKernelContext *ctx) override {
      ...
      OP_REQUIRES_OK(ctx,
                     ctx->allocate_output(2, TensorShape({ batch_size, 6 }),  &spat_transform_a_t));
      OP_REQUIRES_OK(ctx,
                     ctx->allocate_output(3, TensorShape({ batch_size, 6 }), &spat_transform_b_t));

      // Allocate temporary pinned memory for the spatial transforms to be used
      // on the GPU
      ...
      OP_REQUIRES_OK(ctx,
                     ctx->allocate_temp(DataTypeToEnum<float>::value,
                                        ...
                                        &spat_transform_a_pinned_t, pinned_allocator));
      OP_REQUIRES_OK(ctx,
                     ctx->allocate_temp(DataTypeToEnum<float>::value,
                                        TensorShape({ batch_size, 6 }),
                                        &spat_transform_b_pinned_t, pinned_allocator));
      auto spat_transform_a_pinned = spat_transform_a_pinned_t.tensor<float, 2>();
      auto spat_transform_b_pinned = spat_transform_b_pinned_t.tensor<float, 2>();
      ...
      // Copy spatial coefficients A to the output Tensor on the CPU (output for
      // FlowAugmentation)
