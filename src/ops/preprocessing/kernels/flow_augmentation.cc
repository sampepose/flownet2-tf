...
class FlowAugmentation : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    const Tensor& transforms_a_t = ctx->input(1);
    const Tensor& inv_transforms_b_t = ctx->input(2);
    ...
    auto transforms_a = transforms_a_t.tensor<float, 2>();
    auto inv_transforms_b = inv_transforms_b_t.tensor<float, 2>();
