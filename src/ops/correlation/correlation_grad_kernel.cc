...
class CorrelationGradKernel : public OpKernel {
  public:
    ...
    void Compute(OpKernelContext *ctx) override {
      ...
      const int padded_in_height = input_a_t.dim_size(1);
      const int padded_in_width  = input_a_t.dim_size(2);
      const int in_channels      = input_a_t.dim_size(3);
      const int in_height        = in_height - 2 * pad;
      const int in_width         = in_width - 2 * pad;
      const int in_count         = batch_size * in_height * in_width * in_channels;
      ...
      const int out_height = ceil((float)(padded_in_height - border_size * 2) / (float)stride_1);
      const int out_width  = ceil((float)(padded_in_width - border_size * 2) / (float)stride_1);
